...
-(void) setupMIDI {
   ...
   // outPort = 0;
    ...
    MIDIInputPortCreate(client, CFSTR("Input port"), MyMIDIReadProc, CFBridgingRetain(self), &inPort);
//	MIDIOutputPortCreate(client, (CFStringRef)@"Output Port", &outPort);
    ...
    MIDIDeviceListRef  ref;
    MIDIDeviceListGetNumberOfDevices(ref);
    if (sourceCount<2) {
        
       // [self showNoMidiAlert];
    }
	for (int i = 0; i < sourceCount; ++i) {
        ...
        NSLog(@"Midi Name == %@",endpointName);
        
        CFStringRef manufactuer = NULL;
         nameErr = MIDIObjectGetStringProperty(src, kMIDIPropertyManufacturer, &endpointName);
        
        NSLog(@"Manufactuer == %@",manufactuer);

        CFStringRef displayName = NULL;
        nameErr = MIDIObjectGetStringProperty(src, kMIDIPropertyDisplayName, &displayName);
...
-(void)showNoMidiAlert
{
    UIAlertView  *alert=[[UIAlertView alloc]initWithTitle:@"No Midi Device" message:@"No Midi Device has been detected. Please connect a groovtube device" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
